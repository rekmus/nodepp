...
bool Cusers_p_resets::slots_[CDB_MAX_INSTANCES]={0};


/* ---------------------------------------------------------------------------
   Constructor
--------------------------------------------------------------------------- */
Cusers_p_resets::Cusers_p_resets()
{
    setInstance(slots_);

    table_ = "users_p_resets";
...
bool Cusers_p_resets::Fetch()
{
    ...
    if ( ret != 0 )
    {
        ...
        {
            Cdb::ThrowSQL("Cusers_p_resets::Fetch | mysql_stmt_fetch");
            return false;
        }
    }
...
--------------------------------------------------------------------------- */
bool Cusers_p_resets::Get(const std::string& arg_linkkey)
{
    int ret;
    ...
    {
        char q[CDB_SQLBUF];
        sprintf(q, "SELECT linkkey,user_id,created,tries FROM users_p_resets WHERE linkkey=BINARY ?");
        ret = mysql_stmt_prepare(sGet_, q, strlen(q));
        if ( ret != 0 )
            Cdb::ThrowSQL("Cusers_p_resets::Get | mysql_stmt_prepare");
        firstGet_ = false;
    }
    ...
    if ( mysql_stmt_execute(sGet_) )
        Cdb::ThrowSQL("Cusers_p_resets::Get | mysql_stmt_execute");

    bindOutput(sGet_);

    if ( mysql_stmt_store_result(sGet_) )
        Cdb::ThrowSQL("Cusers_p_resets::Get | mysql_stmt_store_result");
    ...
    if ( ret != 0 )
    {
        if ( ret == 1 || ret == MYSQL_NO_DATA )
            ...
            Cdb::ThrowSQL("Cusers_p_resets::Get | mysql_stmt_fetch");
    }
...
/* ---------------------------------------------------------------------------
   Insert record
--------------------------------------------------------------------------- */
unsigned Cusers_p_resets::Insert()
{
    int ret;
    ...
    {
        char q[CDB_SQLBUF];
        sprintf(q, "INSERT INTO users_p_resets (linkkey,user_id,created,tries) VALUES (?,?,?,?)");
        ret = mysql_stmt_prepare(sInsert_, q, strlen(q));
        if ( ret != 0 )
            Cdb::ThrowSQL("Cusers_p_resets::Insert | mysql_stmt_prepare");
        firstInsert_ = false;
    }
    ...
    if ( ret != 0 )
        Cdb::ThrowSQL("Cusers_p_resets::Insert | mysql_stmt_execute");
...
/* ---------------------------------------------------------------------------
   Update record by PK
--------------------------------------------------------------------------- */
void Cusers_p_resets::Update(const std::string& arg_linkkey)
{
    int ret;
    ...
    {
        char q[CDB_SQLBUF];
        sprintf(q, "UPDATE users_p_resets SET linkkey=?,user_id=?,created=?,tries=? WHERE linkkey=BINARY ?");
        ret = mysql_stmt_prepare(sUpdate_, q, strlen(q));
        if ( ret != 0 )
            Cdb::ThrowSQL("Cusers_p_resets::Update | mysql_stmt_prepare");
        firstUpdate_ = false;
    }
    ...
    if ( ret != 0 )
        Cdb::ThrowSQL("Cusers_p_resets::Update | mysql_stmt_execute");
}
...
/* ---------------------------------------------------------------------------
   Delete record by PK
--------------------------------------------------------------------------- */
void Cusers_p_resets::Delete(const std::string& arg_linkkey)
{
    int ret;
    ...
    {
        char q[CDB_SQLBUF];
        sprintf(q, "DELETE FROM users_p_resets WHERE linkkey=BINARY ?");
        ret = mysql_stmt_prepare(sDelete_, q, strlen(q));
        if ( ret != 0 )
            Cdb::ThrowSQL("Cusers_p_resets::Delete | mysql_stmt_prepare");
        firstDelete_ = false;
    }
    ...
    if ( ret != 0 )
        Cdb::ThrowSQL("Cusers_p_resets::Delete | mysql_stmt_execute");
}
...
--------------------------------------------------------------------------- */
void Cusers_p_resets::Set(const std::string& arg_linkkey)
{
    int ret;
    ...
    {
        char q[CDB_SQLBUF];
        sprintf(q, "SELECT linkkey FROM users_p_resets WHERE linkkey=BINARY ?");
        ret = mysql_stmt_prepare(sSet_, q, strlen(q));
        if ( ret != 0 )
            Cdb::ThrowSQL("Cusers_p_resets::Set | mysql_stmt_prepare");
        firstSet_ = false;
    }
    ...
    if ( mysql_stmt_execute(sSet_) )
        Cdb::ThrowSQL("Cusers_p_resets::Set | mysql_stmt_execute");

    bindSetOutput();

    if ( mysql_stmt_store_result(sSet_) )
        Cdb::ThrowSQL("Cusers_p_resets::Set | mysql_stmt_store_result");
    ...
    else if ( ret == 1 || ret == MYSQL_NO_DATA )   /* not found ==> insert new */
    {
        ...
        Cdb::ThrowSQL("Cusers_p_resets::Set | mysql_stmt_fetch");
}
...
/* ---------------------------------------------------------------------------
   Bind key values
--------------------------------------------------------------------------- */
void Cusers_p_resets::bindKey(MYSQL_STMT *s, const std::string& arg_linkkey)
{
    ...
    if ( mysql_stmt_bind_param(s, bndk_) )
        Cdb::ThrowSQL("Cusers_p_resets::bindKey | mysql_stmt_bind_param");
}
...
/* ---------------------------------------------------------------------------
   Bind input values
--------------------------------------------------------------------------- */
void Cusers_p_resets::bindInput(MYSQL_STMT *s, bool withKey, const std::string& arg_linkkey)
{
    ...
    bndi_[3].buffer_type = MYSQL_TYPE_SHORT;
    bndi_[3].buffer = (char*)&tries;
    ...
    if ( mysql_stmt_bind_param(s, bndi_) )
        Cdb::ThrowSQL("Cusers_p_resets::bindInput | mysql_stmt_bind_param");
}
...
/* ---------------------------------------------------------------------------
   Bind output values
--------------------------------------------------------------------------- */
void Cusers_p_resets::bindOutput(MYSQL_STMT *s)
{
    ...
    bndo_[3].buffer_type = MYSQL_TYPE_SHORT;
    bndo_[3].buffer = (char*)&tries;
    bndo_[3].is_null = &tries_is_null_;

    if ( mysql_stmt_bind_result(s, bndo_) )
        Cdb::ThrowSQL("Cusers_p_resets::bindOutput | mysql_stmt_bind_result");
}
...
--------------------------------------------------------------------------- */
void Cusers_p_resets::bindSetOutput()
{
static USERS_P_RESETS_LINKKEY linkkey;    /* to be scrapped anyway */
    ...
    if ( mysql_stmt_bind_result(sSet_, bndso_) )
        Cdb::ThrowSQL("Cusers_p_resets::bindSetOutput | mysql_stmt_bind_result");
}
...
--------------------------------------------------------------------------- */
void Cusers_p_resets::genDTStrings()
{
    if ( created_is_null_ )
        created[0] = EOS;
    else
